...
class Zomato:
    ...
    def restaurant_search(self, query="", latitude="", longitude="", cuisines="", cft=1):
        """
        ...
        headers = {'Accept': 'application/json', 'user-key': self.user_key}
        r = (requests.get(base_url + "search?cft="+ str(cft) + "&lat=" + str(latitude) + "&lon=" + str(longitude) + "&cuisines=" + str(cuisines), headers=headers).content).decode("utf-8")
